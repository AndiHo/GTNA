 ...
 *
 */
package gtna.transformation.hyperbolicEmbeddings;
...
public class KBEmbedding extends Transformation{
	...
	private Apcomplex[] computeSigma(int deg){
		...
		// Caution: a,b,c,d name here the entries of the 2*2 matrix which represents the hperbolic isometry sigma
...
//		
	...
	private int maxDegreeSpanningTree(Graph g, int rootIndex){
		...
		while(!nodequeue.isEmpty()){
			...
			// initialize the spanning tree degree with the degree in the full graph
			int degree = outgoing.length; 
			for (int n: outgoing){
				if (!visited[n]){
					visited[n] = true;
					nodequeue.add(nodes[n]);	
					...
					// does not count towards the node degree
					degree--;
				}
			}
			if (degree > maxDegree){
